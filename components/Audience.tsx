        ...
        <div className="lg:w-2/3 grid grid-cols-1 md:grid-cols-3 gap-6">
          {topVideos.map((video) => (
            <div key={video.id} className="group relative h-full">
              ...
              <div className="h-full p-6 pt-12 pb-8 flex flex-col relative z-10">
                {/* Screen Header */}
                ...
                {/* Metric Hero - Views */}
                <div className="flex items-center gap-2 text-3xl font-bold text-white mb-6">
                  <Play size={24} className="fill-current text-pink-500" />
                  {video.views}
                </div>

                {/* Detailed Stats Grid */}
                <div className="grid grid-cols-3 gap-2 border-t border-b border-slate-700/50 py-4 mb-4">
                  <div className="flex flex-col items-center justify-center text-center">
                    <div className="flex items-center gap-1 text-slate-400 mb-1">
                      <Heart size={14} />
                    </div>
                    <span className="text-sm font-bold text-white">{video.likes}</span>
                  </div>
                  <div className="flex flex-col items-center justify-center text-center border-l border-slate-700/50">
                    <div className="flex items-center gap-1 text-slate-400 mb-1">
                      <MessageCircle size={14} />
                    </div>
                    <span className="text-sm font-bold text-white">{video.comments}</span>
                  </div>
                  <div className="flex flex-col items-center justify-center text-center border-l border-slate-700/50">
                    <div className="flex items-center gap-1 text-slate-400 mb-1">
                      <Bookmark size={14} />
                    </div>
                    <span className="text-sm font-bold text-white">{video.saves}</span>
                  </div>
                </div>

                <div className="flex-grow">
                  <p className="text-slate-400 text-xs leading-relaxed">
                    <span className="text-accent-teal font-bold text-[10px] uppercase block mb-1">Strategy & Analysis</span>
                    {video.analysis}
                  </p>
                </div>
